...
SpeedDistance
Integrate_EulerDirect(const MotionStatePair &state, double v)
{
    double dt = state.DT();
...
SpeedDistance
Integrate_RK4(const MotionStatePair &state, double v)
{
    double t0 = state.T0();
    double dt = state.DT();
...
SpeedDistance
Integrate_RK4_38(const MotionStatePair &state, double v)
{
    double t0 = state.T0();
    double dt = state.DT();
...
SpeedDistance SymplecticSum(int taps, const MotionStatePair& state, double v, const double * d, const double * c)
{
#if defined(TEST_COEFFICIENTS)
...
SpeedDistance Integrate_Yoshida4(const MotionStatePair &state, double v)
{
...
// Received 11 July 1990; accepted for publication 11 September 1990 Communicated by D.D.Holm
// http://cacs.usc.edu/education/phys516/Yoshida-symplectic-PLA00.pdf
SpeedDistance Integrate_Yoshida6(const MotionStatePair &state, double v)
{
...
SpeedDistance Integrate_Yoshida8(const MotionStatePair &state, double v)
{
...
SpeedDistance Integrate_BlanesMoan(const MotionStatePair &state, double v)
{
...
SpeedDistance Integrate_Hairer8(const MotionStatePair &state, double v)
{
    //static const double s_W0 = (1 - 2 * (s_W1 + s_W2 + s_W3 + s_W4 + s_W5 + s_W6 + s_W7));
...
struct Integrator
{
    enum eIntegrator {EulerDirect = 0, RK4, RK4_38, Yoshida4, Yoshida6, Yoshida8, BlanesMoan, Hairer8};
    static SpeedDistance I(const MotionStatePair &state, double v, eIntegrator e = Hairer8)
    {
        typedef SpeedDistance(*PIntegrator)(const MotionStatePair&, double);
        static const PIntegrator fpa[] = {
            Integrate_EulerDirect,
            Integrate_RK4,
            Integrate_RK4_38,
            Integrate_Yoshida4,
            Integrate_Yoshida6,
...
SpeedDistance
Bicycle::SampleSpeed(BicycleSimState &newState)
{
    // Detect and filter obvious power spikes.
    newState.Watts() = FilterWattIncrease(newState.Watts());
    ...
    SpeedDistance out = Integrator::I(state, m_Velocity);
