...
typedef int (*Prototype_EzUsb_control_msg)(libusb_device_handle *, int, int, int, int, char *, int, int);
 ...
 */
extern int libezusb_load_ram(libusb_device_handle *device,
    const char *path, int fx_type, int img_type, int stage, Prototype_EzUsb_control_msg uptr);

/*
 * This function performs a specialized firmware load for the
 * earlier 1902 model as used by the Tacx Imagic
 */
extern int libezusb_load_ram_imagic (libusb_device_handle *device,
    const char *path, Prototype_EzUsb_control_msg uptr);
