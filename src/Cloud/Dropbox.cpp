...
bool Dropbox::createFolder(QString path)
{
    ...
    QByteArray data;
    data.append(QString("{ \"path\": \"%1\", \"autorename\": false }").arg(path).toLatin1());
...
Dropbox::readdir(QString path, QStringList &errors)
{
    ...
    while (listHasMoreEntries) {
        ...
        if (firstRequest) {
            data.append(QString("{ \"path\": \"%1\", \"recursive\": false ,\"include_deleted\": false }").arg(path).toLatin1());
            firstRequest = false;
        } else {
            request.setUrl(QUrl("https://api.dropboxapi.com/2/files/list_folder/continue"));
            data.append(QString("{ \"cursor\": \"%1\" }").arg(cursor).toLatin1());
        }
        QNetworkReply *reply = nam->post(request, data);
