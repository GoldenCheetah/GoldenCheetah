...
TodaysPlan::readdir(QString path, QStringList &errors, QDateTime from, QDateTime to)
{
    ...
    while (offset < resultCount) {
        ...
        if (offset == 0) {
            ...
            QByteArray jsonStringInUTF8 = jsonString.toUtf8();

            printd("request: %s\n", jsonStringInUTF8.toStdString().c_str());

            QByteArray jsonStringDataSize = QByteArray::number(jsonStringInUTF8.size());

            request.setHeader(QNetworkRequest::ContentTypeHeader,"application/json");
            request.setRawHeader("Content-Length", jsonStringDataSize);
            reply = nam->post(request, jsonStringInUTF8);
        } else {
