...
{
	// deletes all of roots children, each child destructor deletes its children
	delete rootItem_;
}
...
EquipmentModel::deleteEquipmentsRefsMatching(const EquipmentNode* eqNode)
{
	...
	// Depending on how the children have been moved there maybe
	// some collateral damage but they are just references
	findAndDeleteMatchingEqRefs(rootItem_, eqNode);

	emit layoutChanged();
}

void // recursive function! 
EquipmentModel::findAndDeleteMatchingEqRefs(EquipmentNode* eqNodeTree, const EquipmentNode* matchEqNode) {

	int n = 0;
	while (n < eqNodeTree->childCount())
	{
		bool keepSearching = true;
		EquipmentNode* eqRef = eqNodeTree->child(n);
		...
		if (keepSearching) {
			findAndDeleteMatchingEqRefs(eqRef, matchEqNode); n++;
		}
	}
}
...
EquipmentModel::copyItemTreeToRefTree(EquipmentNode* copyNodeTree) {
	...
	if (copyNodeTree->getEqNodeType() == eqNodeType::EQ_DIST_ITEM) {
...
EquipmentModel::equipmentAdded(EquipmentNode* eqParent, int eqToAdd) {
	...
	case eqNodeType::EQ_ITEM_REF: {
		qDebug() << "Trying to add an equipment reference node!";
	} break;
