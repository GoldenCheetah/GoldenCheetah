...
Result Leaf::eval(DataFilterRuntime *df, Leaf *leaf, float x, RideItem *m, RideFilePoint *p, const QHash<QString,RideMetric*> *c, Specification s)
{
 ...
 #endif
 #ifdef GC_WANT_R
        if (leaf->function == "R")  return Result(df->runRScript(m->context, *leaf->lvalue.s, m, c, s));
 #endif
        return Result(0);
    }
    break;
...
#ifdef GC_WANT_R
double
DataFilterRuntime::runRScript(Context *context, QString script, RideItem *m, const QHash<QString,RideMetric*> *metrics, Specification spec)
{
    ...
    // run it !!
    rtool->scriptContext = ScriptContext(context, m, metrics, spec);
    rtool->canvas = NULL;
    rtool->chart = NULL;
    ...
    // clear context
    rtool->scriptContext = ScriptContext();
