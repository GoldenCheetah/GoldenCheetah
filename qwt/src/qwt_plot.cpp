        ...
        if ( axisEnabled( axisId ) )
        {
            ...
            if ( axisId == xBottom || axisId == xTop )
            {
                ...
                if ( axisEnabled( yLeft1 ) )
                    r = r.subtract( QRegion( scaleRect[yLeft1] ) );
                if ( axisEnabled( yLeft2 ) )
                    r = r.subtract( QRegion( scaleRect[yLeft2] ) );
                if ( axisEnabled( yLeft3 ) )
                    r = r.subtract( QRegion( scaleRect[yLeft3] ) );

                if ( axisEnabled( yRight ) )
                    r = r.subtract( QRegion( scaleRect[yRight] ) );

                if ( axisEnabled( yRight1 ) )
                    r = r.subtract( QRegion( scaleRect[yRight1] ) );
                if ( axisEnabled( yRight2 ) )
                    r = r.subtract( QRegion( scaleRect[yRight2] ) );
                if ( axisEnabled( yRight3 ) )
                    r = r.subtract( QRegion( scaleRect[yRight3] ) );
