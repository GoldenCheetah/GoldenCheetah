...
template <typename T>
void QwtPlotSeriesItem<T>::setData(const double *xData, const double *yData, int size)
{
    QwtPointArrayData *data = new QwtPointArrayData(xData, yData, size);
    setData(data);
}

template <typename T>
void QwtPlotSeriesItem<T>::setData( const QVector<double> &x, const QVector<double> &y )
{
